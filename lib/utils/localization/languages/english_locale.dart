import 'package:sfera_project_1/presentation/template/template.dart';

class English {
  static const locale = {
    ConstantText.changeLanguage: 'Change language',
    ConstantText.changeTheme: 'Change theme',
    ConstantText.back: 'Back',
    ConstantText.enterInAccount: 'Sign in',
    ConstantText.forNewUser: 'If you are a new user, you must register',
    ConstantText.register: 'Register',
    ConstantText.verificationToContinue: 'You need to be verified to continue',
    ConstantText.passVerification: 'Pass verification',
    ConstantText.username: 'Username',
    ConstantText.password: 'Password',
    ConstantText.login: 'Login',
    ConstantText.resetPassword: 'Reset the password',
    ConstantText.russian: 'Russian',
    ConstantText.english: 'English',
    ConstantText.japanese: 'Japanese',
    ConstantText.light: 'Light',
    ConstantText.dark: 'Dark',
  };
}
